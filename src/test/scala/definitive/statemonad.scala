...
def insert(coin: Coin): State[Machine, Unit] = 
  ...
  def insertOld(coin: Coin): State[Machine, Unit] = State(m =>
    ( m.copy(coins = m.coins + 1), () )
  )
...
def insertSet(coin: Coin): State[Machine, Unit] =
  for {
    m <- State.get[Machine]
...
def insertModify(coin: Coin): State[Machine, Unit] = 
  State.modify(m => m.copy(coins = m.coins + 1))

  def insertModify2(coin: Coin): State[Machine, Unit] = for {
...
// describe
val insertCoin: State[Machine, Candy] = Machine.turn()
...
val program = Machine.insert(Coin()).flatMap(_ => turn())
...
candy shouldBe Candy(RED)
    ...
    "test different flavours of inserting a Coin" in {
      ...
      insert(Coin()) run m shouldBe (Machine(1, candies), ())  
      insertSet(Coin()) run m shouldBe (Machine(1, candies), ())  
      insertModify(Coin()) run m shouldBe (Machine(1, candies), ())  
...
object Foo {
  ...
  object Machine {
    ...
    def turn() = State[Machine, Candy](m =>
      ( m.copy(candies = m.candies.tail), m.candies.head )
    )

    def insert(coin: Coin) =
      modify[Machine](m => m.copy(coins = m.coins + 1))
  }
}
object Baz {
  case class Machine(coins: Int, candies: List[Candy]) {
